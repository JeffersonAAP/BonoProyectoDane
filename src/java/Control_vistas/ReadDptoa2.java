 ...
 * @author JeAnA
 */
public class ReadDptoa2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for (Municipios var : mun)
        {
            int cant = var.getPersonaList().size();
            request.getSession().setAttribute("cant", cant);
        }
        
        request.getSession().setAttribute("municipios", mun);
        request.getRequestDispatcher("a21.jsp").forward(request, response);
    }
